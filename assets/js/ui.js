  ...
  resultado.forEach(({ categoria, resultados }) => {
    ...
    if (contenedorDestino && resultados.length > 0) {
      ...
      resultados.forEach((objeto) => {
        ...
        const yaEsFavorito = listaFavoritosInicial.some(
          (fav) => fav.tarjetaId === objeto.id,
        );
        ...
        boton.addEventListener("click", async () => {
          ...
          const esFavAhora = favsActualizados.some(
            (fav) => fav.tarjetaId === objeto.id,
          );
          if (esFavAhora) {
...
const mostrarMensajeBusquedaVacia = () => {
  ...
  if (!seccionMensaje) {
    ...
    texto.textContent =
      "Utiliza el buscador de la cabecera para encontrar personajes, objetos, lugares y mucho más. ¿A qué esperas?";
...
const gestionarBusqueda = () => {
  ...
  let categoriasActivas = [...checkboxes]
    .filter((checkbox) => checkbox.checked)
    .map((checkbox) => checkbox.dataset.categoria);

  if (categoriasActivas.length === 0) {
    categoriasActivas = [...checkboxes].map(
      (checkbox) => checkbox.dataset.categoria,
    );
  }
  ...
  temporizadorDebouncer = setTimeout(async () => {
    const resultado = await obtenerResultadosDesdeAPI(
      textoBusqueda,
      categoriasActivas,
    );
